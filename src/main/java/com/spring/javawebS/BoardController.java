...
public class BoardController {
	...
	public String boardInputPost(BoardVO vo) {
		...
		// 이미지들의 모든 복사작업을 마치면, ckeditor폴더경로를 board경로로 변경한다. (/rosurces/data/ckeditor/ ==> /rosurces/data/board/)
		vo.setContent(vo.getContent().replace("/data/ckeditor/", "/data/board/")); // 자바 replace는 모조리 다 찾아 바꿔줌
		
		// contetnt안의 내용정리가 끝나면 변경된 vo를 DB에 저장시켜준다.
	...
	// 글 내용 상세보기
	@SuppressWarnings("unchecked") // 노란줄 생겨서 올림
	...
	public String boardContentGet(
		...
		if(!contentIdx.contains(imsiContentIdx)) {
			boardService.setBoardReadNum(idx); // 조회수 1 증가하기
